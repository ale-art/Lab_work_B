...
i=1;
sim_result=cell(1,2*2*2*2*2);
% fprintf("*** Simulink progress ***");
for tau_1_0=tau_1(1):diff(tau_1)/1:tau_1(2)
    %Gu1_0=k1/(tau_1_0*s+1);
    for k_2_0=k_2(1):diff(k_2)/1:k_2(2)
        for tau_2_0=tau_2(1):diff(tau_2)/1:tau_2(2)
            for zeta_0=zeta(1):diff(zeta)/1:zeta(2)
                %Gu2_0=k_2_0/(tau_2_0^2*s^2+2*zeta_0*tau_2_0*s+1);

                for tau_3_0=tau_3(1):diff(tau_3)/1:tau_3(2)
